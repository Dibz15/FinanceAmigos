...
Here, we shall compare the results obtained based upon the various signals generated by our classmates from Group 28 which includes our generated LSTM signals and compares the performances of the trading strategies of the stocks over the period of '01-01-2022' and '01-01-2023'. To give a basic overview, based upon the optimization of Particle Swarm Optimization (PSO), they have considered various technical trading signals and based upon it, they are buying and selling stocks. By analysing the signals, they are deciding to buy/sell one share at a time.

```{r, out.width='65%', fig.align='center', echo=FALSE}
knitr::include_graphics("./comparison_2.png")
```
```{r, out.width='65%', fig.align='center', echo=FALSE}
knitr::include_graphics("./comparison_1.png")
```

Our approach, in contrast, focuses on buying stocks based on the predicted profit percentage rather than purchasing one share at a time. Through a series of experiments, we discovered that our LSTM model performs better in bear markets when trading the 'SPY' stock. It outperforms the grammar method developed by Group 27 on SPY, and we believe this is because it maintains more flexibility because it is trading stocks based on a proportion that is calculated from the predicted profit levels. 

However, in bullish markets such as 'QQQ', the LSTM's performance was not as strong compared to the buy_hold, grammar, and common signals, with buy_hold yielding the best value over time. We observed that the LSTM signal flattens starting from Q3 and completely flattens in Q4, indicating that our algorithm advises against taking any action on the stock during these periods. This could be because our network was only trained on a single asset (AMD), and would perhaps perform better if also trained on the index (SPY) data. 
