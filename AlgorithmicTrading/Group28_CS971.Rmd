...
```
Finally we obtain the signals. We will plot them in the evaluation section.
...
# Compare results with the first project

Accessing signal data in the form of a csv file

```{r}
zestin_signal_spy <- read.csv("predicted_days_spy.csv")
```

Compare the signals column data with the `test_all_signals` dataframe

```{r}
# Replace the column values 'buy' with 1, 'sell' with -1 and 'hold' with 0
# austin_signal_data$Signals <- as.numeric(austin_signal_data$Signals)
zestin_signal_spy$Signals[zestin_signal_spy$Signals == 'Buy'] <- 1
zestin_signal_spy$Signals[zestin_signal_spy$Signals == 'Sell'] <- -1
zestin_signal_spy$Signals[zestin_signal_spy$Signals == 'Hold'] <- 0
```

```{r}
# Check the length of the austin signal data and append the signals column to the test_all_signals dataframe
if (length(zestin_signal_spy$Signals) == nrow(test_all_signals)) {
    test_all_signals <- cbind(test_all_signals, zestin_signal_spy$Signals)
    colnames(test_all_signals)[ncol(test_all_signals)] <- "lstm"
}else {
    test_all_signals <- cbind(test_all_signals, zestin_signal_spy$Signals[1:nrow(test_all_signals)])
    colnames(test_all_signals)[ncol(test_all_signals)] <- "lstm"
}
```
...
# test_all_signals <- test_all_signals[complete.cases(test_all_signals), ]
test_all_signals[is.na(test_all_signals)] <- 0
...
```{r}
zestin_signal_qqq <- read.csv("predicted_days_qqq.csv")
```
...
# Replace the column values 'buy' with 1, 'sell' with -1 and 'hold' with 0
# austin_signal_data$Signals <- as.numeric(austin_signal_data$Signals)
zestin_signal_qqq$Signals[zestin_signal_qqq$Signals == 'Buy'] <- 1
zestin_signal_qqq$Signals[zestin_signal_qqq$Signals == 'Sell'] <- -1
zestin_signal_qqq$Signals[zestin_signal_qqq$Signals == 'Hold'] <- 0
```

```{r}
# Check the length of the austin signal data and append the signals column to the qqq_all_signals dataframe
if (length(zestin_signal_qqq$Signals) == nrow(qqq_all_signals)) {
    qqq_all_signals <- cbind(qqq_all_signals, zestin_signal_qqq$Signals)
    colnames(qqq_all_signals)[ncol(qqq_all_signals)] <- "lstm"
}else {
    qqq_all_signals <- cbind(qqq_all_signals, zestin_signal_qqq$Signals[1:nrow(qqq_all_signals)])
    colnames(qqq_all_signals)[ncol(qqq_all_signals)] <- "lstm"
}
```
