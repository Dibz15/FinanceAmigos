...
```
The above plot shows the closing prices of the SPY index from 2018 to 2021. This will be our training data. For our testing data, we will follow two parallel approaches: changing the dates (`test_year`), and changing the index (`test_index`).
...
The above plot shows the closing prices of the SPY index from 2022 to 2023, which is the first test data.
...
In `get_grammar_expr` we use the grammar to create a grammatical evolution algorithm, which will find the best expression among addition, subtraction and multiplication that maximises profit. We then use this expression to create a signal.

```{r}
add_t <- function (a,b) { return(((a+b+4)%%3 -1))}
sub_t <- function (a,b) { return(((a-b+4)%%3 -1))}
```
...
In `plot_graph_all` we plot all the signals on the same graph. 
```{r echo=F}
...
In `get_investment_values` we use the backtest signal function to get the total value of the investment for each signal. 

In `get_buy_hold` we use the backtest signal function to get the total value of the investment for the buy and hold strategy. 
...
The following code creates a dataframe of all the investment values for each signal using the `get_investment_values` function for the training data.
...
Below, we obtained the majority signal and the grammar signal for the test data (i.e., SPY data from 2022 to 2023). These signals are then combined so that we can plot them all on the same graph and compare.
...
The following code creates a dataframe of all the investment values for each signal using the `get_investment_values` function for the test data (SPY index).
...
Below, we obtain the signals for the test data (i.e., QQQ data from 2022 to 2023).
...
Similar to the previous test, we obtained the majority signal and the grammar signal for the test data. These signals are then combined so that we can plot them all on the same graph and compare.
...
The following code creates a dataframe of all the investment values for each signal using the `get_investment_values` function for the test data (QQQ index).
