...
# Introduction and background (4 marks)
>> Background to the problem - a short review of the approach you are going to employ and a brief overview of related work in the area drawn from either published papers or blog postings

Algorithmic trading refers to any form of trading using sophisticated algorithms to automate all or some part of the trade cycle. The use of technical indicators to figure out the best time to buy and sell stocks has been around for a long time. The most popular technical indicators are the simple moving average (SMA), the exponential moving average (EMA), the relative strength index (RSI), the moving average convergence divergence (MACD), and the Bollinger bands. The MACD is a trend-following momentum indicator that shows the relationship between two moving averages of a security's price. A standard MACD is calculated by subtracting the 26-day exponential moving average (EMA) from the 12-day EMA. The result of that calculation is the MACD line. A nine-day EMA of the MACD called the "signal line," is then plotted on top of the MACD line, which can function as a trigger for buy and sell signals. Traders may buy the security when the MACD crosses above its signal line and sell—or short—the security when the MACD crosses below the signal line. The MACD indicator is used to identify the trend direction as well as the momentum.


The authors in [1] take a combined signal approach to increase profitability by combining the signals from multiple technical indicators. They make use of different trading rules such as the moving average cross-over rule (MAC-O) and trading range break-outs (TRB-O). While the MAC-O rule compares a short-run moving average to a long-run moving average, the TRB-O rule generates a buy signal when the prices cross above the local maximum and a sell signal when the prices cross below the local minimum. The trading rules are applied on the different stock indices like NASDAQ, DJIA, and TSX300 and a majority voting technique is adopted to find the best trading signal. While the results obtained were statistically significant, the authors stress the need to find better parameters for the technical indicators and an AI-based method to implement the combined signal approach.


In [2], the authors propose to combine the technical indicators RSI and MA and optimise them using the genetic algorithms (GA) to find the best trading signal for the foreign exchange market. The focus is on calculating the most appropriate trade timing, rather than predicting trading prices. The study uses machine learning to obtain objective results by utilizing information from past market trends. Trading rules are optimized using genetic algorithms by generating a population of trading rules based on technical indexes. The fitness of each rule is evaluated based on its profitability and stability. The fittest rules are then selected for reproduction, where crossover and mutation operators are applied to create new offspring. This process is repeated for several generations until the optimal trading rule is found. The results are compared with a method that uses a Neural Network and the paper concludes that the proposed GA system outperforms the Neural Network method in terms of profitability and stability.


The different components of algorithmic trading (AT) and the techniques involved are explained in the survey paper [3]. The article discusses two principal approaches to risk management: limiting the amount of risk and limiting the type of risk. The former involves managing exposure through limiting by penalty or constraint, such as leverage, while the latter involves eliminating whole types of exposure. Algorithmic trading involves several steps such as data cleaning, pre-trade analysis, trade signal generation, trade execution, and post-trade handling. Finally, it highlights that the competitive nature of algorithmic trading makes for a secretive community where implementation details are difficult to find. In this project, we are focussing on generating the trade signal and then evaluating them using the backtesting technique for test data.


Add footnotes for this text citing the references.


# Overview of data (2 marks)
>> An overview of the of data you chose to work with (as usual, summary plots would be welcome). Working with one data set is fine but if you want to extend that to consider several assets then that is okay too
...
# Trading strategy (9 marks)
>> Details of your trading strategy and any specific decisions about the representation, fitness function, model configuration etc. along with details about implementation, any key parameters  etc...
...
```{r echo=F}
plot_graph_all <- function(variable, legend, title, xlab, ylab) {
...
colnames(pool_values_all) <- c("small", "med", "big", "common","majority","grammar")
rownames(pool_values_all) <- rownames(all_signals)
```

```{r}
plot_graph_all(pool_values_all, colnames(pool_values_all), "All investment values", "Date")
```
...
```{r}
plot_graph_all(cumsum(qqq_all_signals), colnames(qqq_all_signals), "Signals", "Date")
```
...
colnames(qqq_pool_values_all) <- c("small", "med", "big", "common", "majority", "grammar")
rownames(qqq_pool_values_all) <- rownames(qqq_all_signals)
```

```{r}
plot_graph_all(qqq_pool_values_all, colnames(qqq_pool_values_all), "All investment values", "Date")
```
